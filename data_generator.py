...
class AudioGeneratorCached(AudioGenerator):
    ...
    def featurize(self, audio_clip):
        features = self.file_to_feature.get(audio_clip)
        if features is None:
            features = super().featurize(audio_clip)
            self.file_to_feature[audio_clip] = features
        return features
...
def vis_train_features(index=0, mfcc_classes=13):
    """ Visualizing the data point in the training set at the supplied index """
    # obtain spectrogram

    audio_generator = AudioGenerator(spectrogram=True)
    audio_generator.load_train_data()
    vis_audio_path = audio_generator.train_audio_paths[index]
    vis_spectrogram_feature = audio_generator.normalize(audio_generator.featurize(vis_audio_path))
    # obtain mfcc
    audio_generator = AudioGenerator(spectrogram=False, mfcc_dim=mfcc_classes)
    audio_generator.load_train_data()
    vis_mfcc_feature = audio_generator.normalize(audio_generator.featurize(vis_audio_path))
    # obtain text label
    vis_text = audio_generator.train_texts[index]
    # obtain raw audio
    vis_raw_audio, _ = librosa.load(vis_audio_path)
    # print total number of training examples
    print('There are %d total training examples.' % len(audio_generator.train_audio_paths))
...
def test_gen():
    s_gen = AudioGeneratorCached(spectrogram=True, minibatch_size=1, sort_by_duration=True)
    s_gen.load_train_data('train_corpus.json')
    m_gen = AudioGeneratorCached(spectrogram=False, minibatch_size=1, sort_by_duration=True)
